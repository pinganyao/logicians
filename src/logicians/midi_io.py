...
class MidiInputAdapter:
    ...
    def poll(self, tempo_bpm: float, *, record: bool = True) -> None:
        if not self._port:
            return
    ...
    def capture_bars(
        ...
        *,
        ...
        """Capture exactly `bars` of musical time.

        When ``from_current_position`` is False (default), capture starts from the
        current sync clock — used right after the first sync note in CLI live mode.

        When True, capture the next ``bars`` from the present loop position without
        resetting the sync clock — used when improvising mid-session after sync.
        """
        beats_per_bar = time_signature[0]
        loop_beats = bars * beats_per_bar

        if from_current_position:
            start_beats = self._elapsed_beats(tempo_bpm)
            end_beats = start_beats + loop_beats
            while self._elapsed_beats(tempo_bpm) < end_beats:
                self.poll(tempo_bpm)
                time.sleep(0.001)
            self._flush_active_notes(tempo_bpm)
            raw = [n for n in self._buffer if start_beats <= n.start < end_beats]
            rebased = [
                NoteEvent(
                    track=n.track,
                    pitch=n.pitch,
                    velocity=n.velocity,
                    start=n.start - start_beats,
                    duration=min(n.duration, end_beats - n.start),
                )
                for n in raw
            ]
            return quantize_notes(rebased, beats_per_bar)
    ...
    def reset_capture_buffer(self) -> None:
        """Clear captured notes without resetting the sync clock."""
        self._buffer = []
        self._active = {}


def wait_until_loop_boundary(sync_time: float, loop_duration_sec: float) -> float:
    """Sleep until the next loop boundary after sync_time. Returns that boundary's wall-clock time."""
    now = time.time()
    elapsed = now - sync_time
    if elapsed < 0:
        return sync_time

    position_in_loop = elapsed % loop_duration_sec
    if position_in_loop < 0.001:
        return now

    remaining = loop_duration_sec - position_in_loop
    time.sleep(remaining)
    return sync_time + (int(elapsed / loop_duration_sec) + 1) * loop_duration_sec
...
def loop_position(
    ...
    """Return current loop number, bar, and beat from wall-clock sync."""
    if sync_time is None or loop_duration_sec <= 0:
        return {"loop": 0, "bar": 0, "beat": 0.0}

    elapsed = max(0.0, time.time() - sync_time)
    loop_num = int(elapsed / loop_duration_sec) + 1
    position_in_loop = elapsed % loop_duration_sec
    beat_sec = 60.0 / tempo_bpm
    beats_per_bar = time_signature[0]
    elapsed_beats = position_in_loop / beat_sec
    bar = int(elapsed_beats / beats_per_bar) + 1
    beat = (elapsed_beats % beats_per_bar) + 1.0
    return {"loop": loop_num, "bar": bar, "beat": beat}
